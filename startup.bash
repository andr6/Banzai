...
# grab repository / directory name
...
ss_key="SS_IP="
ss_value=$(sudo docker network inspect $foldername$default | grep -A 3 'stackstorm' | grep 'IPv4' | sed 's/.*: "//' | sed 's/\/.*//')
...
dd_key="DD_IP="
dd_value=$(sudo docker network inspect $foldername$default | grep -A 3 'dojo' | grep 'IPv4' | sed 's/.*: "//' | sed 's/\/.*//')
...
burp_key="BURP_IP="
burp_value=$(sudo docker network inspect $foldername$default | grep -A 3 'burpdock' | grep 'IPv4' | sed 's/.*: "//' | sed 's/\/.*//')
...
sudo docker exec dojo sed -i 's/ALLOWED_HOSTS = \[\]/ALLOWED_HOSTS = \[\"\*\"\]/' /opt/django-DefectDojo/dojo/settings.py

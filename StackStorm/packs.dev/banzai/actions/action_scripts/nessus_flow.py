...
nessus_url = "https://10.142.120.140:8834/"                      # link to Nessus API
headers = {"X-ApiKeys": "accessKey=" + accessKey + ";secretKey="+ secretKey,"Content-Type": "application/json"}

#### POST to API
def post_request(nessus_url, payload):
  r = requests.post(nessus_url, headers=headers, data=json.dumps(payload), verify=False)
  return r.text

#### GET from API
def get_request(nessus_url, payload):
  r = requests.get(nessus_url, headers=headers, data=json.dumps(payload), verify=False)
  return r.text

#### GET scan details
def get_scan_details(scan_id):
  res = get_request(nessus_url+"scans/"+str(scan_id), '')
  scan = json.loads(res)
  return scan['history'][0]['status'], scan['history'][0]['history_id']

#### GET report status
def get_report_status(scan_id, file_id):
  res = get_request(nessus_url+"scans/"+str(scan_id)+"/export/"+str(file_id)+"/status", '')
...
class Nessus_Scan(Action):
  def run(self, url, testid):
    ...
    res = post_request(nessus_url+"scans", new_scan)               # fetch scan creation details
    scan = json.loads(res)                                  # fetch scan id for (2)
    scan_id = scan['scan']['id']
    print scan_id

    # (2) Launch the scan created from (1)
    post_request(nessus_url+"scans/"+str(scan_id)+"/launch", '')

    # (3) Fetch scan details & obtain status + history_id
    status, history_id = get_scan_details(scan_id)
    ...
    # (4) Request report
    res = post_request(nessus_url+"scans/"+str(scan_id)+"/export?history_id="+str(history_id), {"format": "nessus"})
    scan = json.loads(res)
    print scan
    ...
    # (5) Get report status
    status = get_report_status(scan_id, file_number)
    ...
    xml = get_request(nessus_url+"scans/"+str(scan_id)+"/export/"+str(file_number)+"/download", '')
    f.write(xml)
    f.close
